...
SINGLE_RTSP_URL = "rtsp://admin:Rapport%40123@192.168.1.212:554/video/live?channel=1&subtype=0"  #  your camera RTSP
...
# ===== GPS CONFIG =====
DEVICE_ID = "861327081767498"
ECOCOSMO_API = f"https://status.ecocosmogps.com/rapportsoft.php?imei={DEVICE_ID}"
AUTH_TOKEN = "Bearer RjBEAiBrVSgEwjwH1HodDm677wI5Md9VTQAufhb5gLRYPM5DCAIgdFtQjS1DBrQ0DN6RaBgOz_txkq0ZmOTno4nIB1sXxsx7InUiOjUzMDksImUiOiIyMDI2LTEyLTMwVDE4OjMwOjAwLjAwMCswMDowMCJ9"
...
class MKVRecorder:
    ...
    def stop(self):
        ...
        print(f"[{self.kalmar_id}]  MKV recording stopped")
...
class KalmarSession:
    ...
    "pipe:1",
            ...
            self.ffmpeg = subprocess.Popen(
                cmd,
                stdin=subprocess.PIPE if not rtsp_url else None,
            ...
            try:
                ...
                if self.ffmpeg and self.ffmpeg.stdin:
                    try:
                        ...
                        break
                # 2 Send SAME packet to MKV recorder (NEW)
                if self.recorder:
    ...
    def _frame_reader(self):
        ...
        while self.running:
            ...
            if self.frame_counter > 0 and (time.time() - self.last_frame_time > 60):
                print(f"[{self.kalmar_id}]  Stream frozen for 60 seconds")
            ...
            try:
                ...
                # ADDED: Check for data availability before reading
                # import select
                # if hasattr(select, 'poll'):
                #     poller = select.poll()
                #     poller.register(self.ffmpeg.stdout, select.POLLIN)
                #     if not poller.poll(10):  # 10ms timeout
                #         time.sleep(0.001)
                #         continue
                
                # frame_data = self.ffmpeg.stdout.read(frame_size)  
                # frame_data = b""
                ...
                # if len(frame_data) != frame_size:
                #     print(f"[{self.kalmar_id}]  Dropped frame")
                #     continue
                ...
                if len(frame_data) != frame_size:
                    ...
                    print(f"[{self.kalmar_id}]  Incomplete frame ({len(frame_data)}) | errors={consecutive_errors}")
                ...
                if not frame_data:
                    if self.ffmpeg and self.ffmpeg.poll() is not None:
                        print(f"[{self.kalmar_id}]  FFmpeg died  reconnecting")
                ...
                #  POP matching Android timestamp
               #  Assign frame timestamp (SINGLE SOURCE OF TRUTH)
                # with frame_ts_lock:
                #     if frame_ts_buffer[self.kalmar_id]:
                #         frame_ts = frame_ts_buffer[self.kalmar_id].popleft()
                #         ts_source = "android"
                #     else:
                #         frame_ts = monotonic_ms()
                #         ts_source = "local"
                #     main_camera_buffer[self.kalmar_id].append((frame_ts, frame.copy()))
...
#                self.frame_queue.put_nowait((frame, frame_ts))
    ...
    def _restart_ffmpeg(self, delay=5):
        ...
        try:
            ...
            self.start_ffmpeg(rtsp_url=SINGLE_RTSP_URL)
...
def main_display_loop():
    ...
    print("\n Starting MAIN THREAD video display loop...")
...
if __name__ == "__main__":
    ...
    print("  Starting OpenCV display in MAIN THREAD...")
    try:
    ...
    # ==================================================
    #  KEEP SERVICE ALIVE (THIS IS WHAT YOU ADD)
    # ==================================================
    if RUN_AS_SERVICE:
